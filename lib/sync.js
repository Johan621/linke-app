  ...
  try {
    ...
    if (remoteFolders) {
      ...
      for (const rf of remoteFolders) {
        // Find if we have a local folder with the same name but different ID (e.g. default seeded folders)
        const duplicate = allLocalFolders.find(f => f.name.toLowerCase() === rf.name.toLowerCase() && f.id !== rf.id);
        if (duplicate) {
          // Migrate any local links using the duplicate folder ID to the official remote ID
          await db.links.where("folderId").equals(duplicate.id).modify({ folderId: rf.id });
          // Delete the local duplicate
          await db.folders.delete(duplicate.id);
        }
